...
export default function StockChart({ ticker, period, chartData, loading }) {
  ...
  // Mirrors of state kept as refs so effect closures are never stale
  const dataRef       = useRef(chartData)
  const indicatorsRef = useRef({ vol: true, ma20: false, ma50: false, ma200: false })
  ...
  // ── Build (or rebuild) the whole chart whenever chartType changes ────────
  // This runs on mount AND whenever the user switches Candle/Line/Area.
  useEffect(() => {
    ...
    if (chartType === 'Candle') {
      ...
      series = chart.addLineSeries({ color: '#4f6ef7', lineWidth: 2 })
    } else {
    ...
    if (data?.length) {
      series.setData(chartType === 'Candle' ? data : data.map(d => ({ time: d.time, value: d.close })))
      ...
      Object.entries(maRefs.current).forEach(([key, s]) => {
        const cfg = MA_CONFIG[key]
        if (cfg) s.setData(calcMA(data, cfg.period))
      })
  ...
  // ── Push new data to all series ──────────────────────────────────────────
  useEffect(() => {
    if (!chartData?.length || !mainRef.current) return
  ...
  useEffect(() => {
    ...
    // MAs — create or destroy
    Object.keys(MA_CONFIG).forEach(key => {
      if (indicators[key] && !maRefs.current[key]) {
        const cfg = MA_CONFIG[key]
        const s   = chart.addLineSeries({
          color: cfg.color, lineWidth: 1.5,
          crosshairMarkerVisible: false,
      ...
      } else if (!indicators[key] && maRefs.current[key]) {
